...
void handle_user_exit(void *data, int dummy)
{
	
	__handle_event(user_exit);
}

void handle_user_enter(void *data, int dummy)
{
	// Starting Tracing
	struct verification *ver = this_cpu_ver();
	if (!verifying(ver)) {
...
struct tp_and_name tps[2] = {
	{
		.probe = handle_user_enter,
		.name = "user_enter",
		.registered = 0
	},
	{
		.probe = handle_user_exit,
		.name = "user_exit",
		.registered = 0
	},
};
